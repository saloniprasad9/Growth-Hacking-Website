...
const ProgramCard = ({img , title ,list1content , list2content , list3content ,index}) => (
    <div className={`flex flex-row p-6 rounded-[10px] w-full bg-green_gradient border border-[rgba(255,255,255,.41)] ${index !== program1.length ? "mb-6" : 'mb-0'} feature-card `}> 
      <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter}`}>
        <img src={img} alt='icon' className='w-[50%] h-[60%]'/>
      </div>
      <div className='flex-1 flex flex-col ml-3 text-[15px] leading-[24px] mb-1'>
        <h2 className='text-white'>{title}</h2>
        <ul key={program1.id}>
            <li className='text-white list-disc'>{list1content}</li>
            <li className='text-white list-disc'>{list2content}</li>
            <li className='text-white list-disc'>{list3content}</li>
        </ul>
      </div>
    </div>
  )
