        ...
        <div className={`${layout.section}`}>
            <div className={`${layout.sectionInfo}`}>
                <div className='flex flex-col w-[550px] px-10 py-10 h-[720px] bg-darkolive_gradient border-[1px] border-[#ddee36] rounded-[14px]'>
                    ...
                    <p className='text-[#c0c0c0]'>Learn from industry experts to develop a growth mindset</p>
                    <div className='flex flex-col space-y-6 mt-10'>
                        <div className='flex flex-row text-left space-x-8 items-center'>
                            <img src={tick3} alt='tick3' className='h-[60px] w-[60px]'/>
                            <p className='text-white text-[30px] font-semibold'>60+ Hours of Content</p>
                        </div>
                        <div className='flex flex-row text-left space-x-8 items-center'>
                            <img src={tick3} alt='tick3' className='h-[60px] w-[60px]'/>
                            <p className='text-white text-[30px] font-semibold]'>15+ Guest Sessions</p>
                        </div>
                        <div className='flex flex-row text-left space-x-8 items-center'>
                            <img src={tick3} alt='tick3' className='h-[60px] w-[60px]'/>
                            <p className='text-white text-[30px] font-semibold'>8 Live QnA</p>
                        </div>
                        <div className='flex flex-row text-left space-x-8 items-center'>
                            <img src={tick3} alt='tick3' className='h-[60px] w-[60px]'/>
                            <p className='text-white text-[30px] font-semibold'>Premium Growth Hacking community of marketers</p>
                        </div>
            ...
            <div className={`${layout.sectionImg} flex flex-col space-y-4`}>
                <h2 className='font-dm font-semibold xs:text-[48px] text-[40px] text-[#ddee36] xs:leading-[76.8px] leading-[66.8px]'>What you get:</h2>
                {benefits.map((benefit,id) => (
                    <div key={id} className='flex flex-row'>
                        <img src={benefit.img} alt='benefit' className='h-[60px] w-[60px]'/>
                        <p className='text-white max-w-[400px]'>{benefit.content}</p>
                    </div>
                ))}
            </div>
        </div>
